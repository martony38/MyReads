...
class BookResults extends Component {
  ...
  getBooksResults() {
    ...
    this.cancelableBookSearch = makeCancelable(
      BooksAPI.search(this.props.query)
    );
    this.cancelableBookSearch.promise
      .then(books => {
        if (typeof books !== "undefined" && !books.error) {
          const newBooks = books.filter(
            book =>
              !this.props.booksOnShelves.map(book => book.id).includes(book.id)
          );
          const booksAlreadyInLibrary = this.props.booksOnShelves.filter(book =>
            books.map(book => book.id).includes(book.id)
          );
          const bookResults = newBooks.concat(booksAlreadyInLibrary);
          this.setState({ books: bookResults });
          this.props.onAddNotice(
            `${bookResults.length} books found (${newBooks.length} new - ${
              booksAlreadyInLibrary.length
            } in library)`
          );
        } else {
          this.setState({ books: [] });
